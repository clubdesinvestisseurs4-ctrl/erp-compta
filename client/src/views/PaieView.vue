...
async function valider(fiche) {
  try {
    ...
    toast.success('Fiche validée et comptabilisée.');
    await load();
  } catch (err) {
    toast.error(err.message);
  }
...
async function supprimer(fiche) {
  if (!(await confirmStore.ask(`Supprimer la fiche de paie de ${fiche.employeNom} pour ${fiche.periode} ?`, { danger: true, confirmLabel: 'Supprimer' }))) return;
  try {
